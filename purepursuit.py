...
Modified to accept a reference path CSV file containing columns: lat,lon,x,y (no header).
The script reads x,y columns (columns index 2 and 3) and uses them as the trajectory.
Usage:
    python3 pure_pursuit_with_csv.py --path reference.csv
or
    python3 pure_pursuit_with_csv.py   # falls back to synthetic path if no file provided

Note: this script expects the provided x,y to be in the same units used by the controller (meters).
If your CSV only contains lat/lon you need to project them to local ENU/x-y coordinates before use.
"""
...
# append project root so `utils.angle` can be resolved when running as script
...
def load_path_from_csv(csv_path):
    """Load a CSV with columns: lat, lon, x, y (no header) and return cx, cy as lists.
    Expects the x,y to be in meters. If CSV is missing or malformed, raise.
    """
    p = Path(csv_path)
    if not p.exists():
    ...
    if data.shape[1] < 4:
        raise ValueError("CSV must have at least 4 columns: lat,lon,x,y")

    cx = data[:, 2].tolist()  # x column (index 2)
    cy = data[:, 3].tolist()  # y column (index 3)
    return cx, cy


def main(path_csv: str = None):
    global is_reverse_mode
    ...
    if path_csv is None:
        ...
        try:
            cx, cy = load_path_from_csv(path_csv)
            print(f"Loaded path from: {path_csv}, loaded {len(cx)} points")
        except Exception as e:
            print(f"Failed to load CSV path ({e}), falling back to synthetic path")
            cx = -1 * np.arange(0, 50, 0.5) if is_reverse_mode else np.arange(0, 50, 0.5)
            cy = [math.sin(ix / 5.0) * ix / 2.0 for ix in cx]
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Pure pursuit with CSV path input')
    parser.add_argument('--path', '-p', dest='path', default=None,
                        help='CSV file path with columns: lat,lon,x,y (no header)')
